...
type Attribute = "hp" | "maxHp" | "mp" | "maxMp" | "hpRegen" | "mpRegen" | "speed" |
"accuracy" | "phyDmg" | "skiDmg" | "phyDef" | "skillDef" | "str" | "int" | "dex" | "con" |
"vit" | "hunger" | "hungerRate"
...
export const AttributeOperator = {
	...
	_(attr: Attribute[], operation: string){
		attr.forEach(a => {
			const operationArr: string[] = operation.split("");
			const operator: string = operationArr[0]
			const isPercentage: boolean = operationArr[operationArr.length-1] === "%" ? true : false;
			let valueBody: number, result: number;
			if(isPercentage)
			...
			//Check rules
			if(result < attributeRules[a].min) result = attributeRules[a].min
			//Some attributerelations
			//When health is > maxHealh | with mp
			if(a === "hp" && result > this.char.attr_health_max) result = this.char.attr_health_max;
			if(a === "mp" && result > this.char.attr_mana_max) result = this.char.attr_mana_max;
			if(a === "hunger" && result > this.char.attr_hunger_max) result = this.char.attr_hunger_max;
